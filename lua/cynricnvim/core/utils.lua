...
local noplugin = vim.list_contains(argv, "--noplugin") or vim.list_contains(argv, "--noplugins") --list_contains检查是否命令行参数中包含"--noplugin"或"--noplugins"
...
--设置{clear = true}防止这段代码在类似init.lua这种配置文件多次被加载或则被手动多次加载那样neovim会
--把ft_group里填入相同命令的重复config
...
utils.get_root = function()
    ...
    local pattern = cynric.chdir_root_pattern
